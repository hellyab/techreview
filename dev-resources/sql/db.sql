...
(
    /* reconsider the id*/
    id          VARCHAR DEFAULT REPLACE(uuid_generate_v4() :: text, '-', ''),
...
(
    id       VARCHAR      NOT NULL PRIMARY KEY DEFAULT REPLACE(uuid_generate_v4() :: text, '-', ''),
    inquirer VARCHAR(250) NOT NULL,
    inquiry  TEXT         NOT NULL,
    FOREIGN KEY (inquirer) REFERENCES user (username)
);
...
(
    ...
    replier VARCHAR(250) NOT NULL,
    answer  VARCHAR      NOT NULL,
    FOREIGN KEY (qid) REFERENCES question (id),
    FOREIGN KEY (replier) REFERENCES user (username)
);